...
try {
        ...
        try {
            ...
            if ($agent_id) {
                $commission_result = safeFetch($conn, 
                    "CALL sp_calculate_agent_commission($agent_id, $reservation_id)");
    ...
    $reservations_result = safeFetch($conn, 
        "SELECT r.reservation_id, c.firstname, c.lastname, l.lot_number, 
         ...
         a.lastname AS agent_lastname, ac.commission_fee, ac.status AS commission_status
         FROM reservation r
                        ...
                        <tbody>
                            <?php foreach ($reservations as $reservation): ?>
                                <tr>
                                    ...
                                    <td class="status-<?= strtolower($reservation['commission_status'] ?? '') ?>">
                                        <?= $reservation['commission_status'] ?? 'N/A' ?>
                                    </td>
